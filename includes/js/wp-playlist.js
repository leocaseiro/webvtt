...
(function ($, _, Backbone) {
	...
	var WPPlaylistView = Backbone.View.extend({
		...
		renderCurrent : function () {
			var dimensions, defaultImage = 'wp-includes/images/media/video.png', i, j, lang, player, webvtt;
			if ( 'video' === this.data.type ) {
				...
				if ( ( player = this.player ) && player.tracks.length ) {
					this.playerNode.empty();
					player.cleartracks( player, player.controls, player.layers, player.media );
				}

				if ( webvtt = this.current.get( 'webvtt' ) ) {
					for ( i = 0, j = webvtt.length; i < j; ++i) {
						this.playerNode.append(
							'<track kind="' + webvtt[i].kind
							+ '" src="' + webvtt[i].src
							+ '" srclang="' + webvtt[i].srclang + '">'
						);
					}
				}
				...
				if ( player && j ) {
					...
					player.controls.children( '.mejs-duration-container' ).after( player.captionsButton );

					if ( lang ) {
						player.setTrack( lang );
						player.captionsButton.find( 'input[type="radio"][value="' + lang + '"]' ).prop( 'checked', true );
					}
				}
			} else {
				if ( ! this.data.images ) {
